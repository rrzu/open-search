...
class OpenSearchClient extends OpenSearch {
    ...
    private function _getHeadersArr($items) {
        $headers = [
            'Content-Type' => $items['content_type'],
            'Date' => $items['date'],
            'Accept-Language' => $items['accept_language'],
            'Content-Md5' => $items['content_md5'],
    ...
    private function _curlEasyswoole($url, $items)
    {
        $method = strtoupper($items['method']);
        $client = new HttpClient();
        $client->setHeaders($this->_getHeadersArr($items),false,false);
        if ($method == self::METHOD_GET) {
            $query = $this->_buildQuery($items['query_params']);
            $url .= preg_match('/\?/i', $url) ? '&' . $query : '?' . $query;
            $req = $client->setUrl($url)->get();
        } else{
            $req = $client->setUrl($url)->post($items['body_json']);
        }
        var_dump($url);
    ...
    private function _curl($url, $items) {
        return $this->_curlEasyswoole($url, $items);
        $method = strtoupper($items['method']);
        $options = array(
